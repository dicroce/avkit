...
vector<stream_info> av_demuxer::get_stream_types() const
{
    return _streamTypes;
}
...
pair<int,int> av_demuxer::get_time_base( int streamIndex ) const
{
    ...
    return make_pair(codecContext->framerate.den, codecContext->framerate.num);
}
    ...
    if( avformat_find_stream_info( _context, NULL ) >= 0 )
    {
        for( int i = 0; i < (int)_context->nb_streams; i++ )
        {
            ...
            else if( _context->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO )
            {
                ...
                if( _bsfc && (_context->streams[i]->codec->codec_id != AV_CODEC_ID_H264) )
                {
                    av_bitstream_filter_close( _bsfc );
                    _bsfc = NULL;
                }
